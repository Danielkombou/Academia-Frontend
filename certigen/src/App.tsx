...
export default function App() {
  ...
  const [recipients, setRecipients] = useState<Recipient[]>([
    { name: 'Daniel Kombou', course: 'Software Engineering Masterclass', date: 'August 2, 2026' },
    { name: 'Alice Smith', course: 'Full-Stack Web Development', date: 'August 2, 2026' },
    { name: 'Bob Johnson', course: 'Advanced React & TypeScript', date: 'August 2, 2026' },
  ])
  const [columns, setColumns] = useState<string[]>(['name', 'course', 'date'])
  ...
  const [textPositions, setTextPositions] = useState({
    name: { x: 50, y: 45, fontSize: 32 },
    course: { x: 50, y: 60, fontSize: 20 },
    date: { x: 50, y: 75, fontSize: 14 },
  })
  ...
  const parseCSV = (csvText: string) => {
    ...
    if (lines.length > 0) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const obj: Recipient = { name: '', course: '', date: '' }
        headers.forEach((h, index) => {
          obj[h] = values[index] || ''
        })
        // Fallbacks
        obj.name = obj[headers[0]] || obj['name'] || 'Recipient'
        obj.course = obj[headers[1]] || obj['course'] || 'Certificate of Completion'
        obj.date = obj[headers[2]] || obj['date'] || new Date().toLocaleDateString()
        parsedRecipients.push(obj)
      }
      if (parsedRecipients.length > 0) {
        setRecipients(parsedRecipients)
        // Auto map first 3 headers if available
        setColumnMapping({
          name: headers[0] || 'name',
          course: headers[1] || 'course',
          date: headers[2] || 'date',
        })
      }
    }
  ...
  const generateCertificatesPDFs = async () => {
    ...
    for (let i = 0; i < recipients.length; i++) {
      ...
      const nameVal = recipient[columnMapping.name] || recipient.name || 'Recipient'
      const courseVal = recipient[columnMapping.course] || recipient.course || 'Course'
      const dateVal = recipient[columnMapping.date] || recipient.date || 'Date'
      ...
      // Add text fields based on percentage positioning (converted to mm)
      ...
      doc.setFontSize(textPositions.course.fontSize)
      const courseX = (textPositions.course.x / 100) * 297
      const courseY = (textPositions.course.y / 100) * 210
      doc.text(courseVal, courseX, courseY, { align: 'center' })

      // Date
      doc.setFontSize(textPositions.date.fontSize)
      const dateX = (textPositions.date.x / 100) * 297
      const dateY = (textPositions.date.y / 100) * 210
      doc.text(dateVal, dateX, dateY, { align: 'center' })
    ...
    <div className="certigen-app min-h-screen bg-white dark:bg-[#16171d] text-[#6b6375] dark:text-[#9ca3af]">
      ...
      <section className="hero-section py-20 px-4 text-center flex flex-col items-center">
        ...
        <p className="hero-subtitle text-lg lg:text-xl text-[#6b6375] dark:text-[#9ca3af] max-w-2xl mb-10 leading-relaxed">
          Upload a template, upload your Excel or CSV file,<br />
          customize text positions, and download hundreds of certificates instantly.
        </p>
      ...
      <section id="generator-flow" className="flow-section max-w-3xl mx-auto py-16 px-4 flex flex-col gap-8">
        {step === 0 && (
          <div className="flow-prompt-card text-center p-8 bg-gray-50 dark:bg-[#1f2028] rounded-2xl border border-gray-200 dark:border-[#2e303a]">
            ...
            <p className="text-base text-[#6b6375]">Click the button above to start uploading your template and data.</p>
          </div>
        )}
        ...
        {step >= 2 && (
          <div className={`step-card p-6 lg:p-8 rounded-2xl border transition-all ${step === 2 ? 'border-[#aa3bff]/50 shadow-xl bg-white dark:bg-[#1f2028]' : step > 2 ? 'border-gray-200 dark:border-[#2e303a] bg-gray-50 dark:bg-[#1f2028]/50' : 'opacity-40 pointer-events-none'}`}>
            <div className="step-header flex items-center gap-4 mb-4">
              ...
              <h3 className="text-xl font-semibold text-[#08060d] dark:text-[#f3f4f6]">Upload CSV/Excel Spreadsheet</h3>
            </div>
            {step === 2 ? (
              <div className="dropzone flex flex-col items-center gap-4">
                <input
                ...
                <label htmlFor="spreadsheet-upload" className="dropzone-label w-full border-2 border-dashed border-gray-300 dark:border-[#2e303a] rounded-xl p-10 text-center cursor-pointer hover:border-[#aa3bff] hover:bg-[#aa3bff]/5 transition-all">
                  <div className="dropzone-icon text-4xl mb-3">📊</div>
                  <p className="text-base font-medium text-[#08060d] dark:text-[#f3f4f6]"><strong>Drop your CSV file here</strong> (Columns: name, course, date)</p>
                  <p className="text-xs text-gray-400 mt-1">Loaded with {recipients.length} demo recipients (or upload your own CSV)</p>
                ...
                <div className="flex justify-between w-full mt-2">
                  <span className="text-xs text-gray-500">{recipients.length} recipients loaded</span>
              ...
              <div className="step-summary flex justify-between items-center text-sm font-medium text-[#08060d] dark:text-[#f3f4f6]">
                <span>📊 {spreadsheetFile?.name || 'recipients.csv'} ({recipients.length} recipients)</span>
        ...
        {step >= 3 && (
          <div className={`step-card p-6 lg:p-8 rounded-2xl border transition-all ${step === 3 ? 'border-[#aa3bff]/50 shadow-xl bg-white dark:bg-[#1f2028]' : step > 3 ? 'border-gray-200 dark:border-[#2e303a] bg-gray-50 dark:bg-[#1f2028]/50' : 'opacity-40 pointer-events-none'}`}>
            <div className="step-header flex items-center gap-4 mb-4">
              ...
              <h3 className="text-xl font-semibold text-[#08060d] dark:text-[#f3f4f6]">Map Columns & Positioning</h3>
            </div>
            {step === 3 ? (
              <div className="mapping-container flex flex-col gap-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="mapping-row flex flex-col gap-1 p-3 bg-gray-50 dark:bg-[#16171d] rounded-lg border border-gray-200 dark:border-[#2e303a]">
                    <label className="text-xs font-semibold uppercase text-gray-500">Certificate Name Field</label>
                    <select
                  ...
                  <div className="mapping-row flex flex-col gap-1 p-3 bg-gray-50 dark:bg-[#16171d] rounded-lg border border-gray-200 dark:border-[#2e303a]">
                    <label className="text-xs font-semibold uppercase text-gray-500">Certificate Course Field</label>
                    <select
                  ...
                  <div className="mapping-row flex flex-col gap-1 p-3 bg-gray-50 dark:bg-[#16171d] rounded-lg border border-gray-200 dark:border-[#2e303a]">
                    <label className="text-xs font-semibold uppercase text-gray-500">Certificate Date Field</label>
                    <select
                ...
                <div className="p-4 bg-gray-50 dark:bg-[#16171d] rounded-xl border border-gray-200 dark:border-[#2e303a] flex flex-col gap-4">
                  <h4 className="text-sm font-semibold text-[#08060d] dark:text-[#f3f4f6]">Text Positioning & Font Size</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                    <div>
                      <label>Name Y Pos ({textPositions.name.y}%)</label>
                      <input type="range" min="20" max="80" value={textPositions.name.y} onChange={e => setTextPositions({...textPositions, name: {...textPositions.name, y: Number(e.target.value)}})} className="w-full mt-1" />
                    </div>
                    <div>
                      <label>Course Y Pos ({textPositions.course.y}%)</label>
                      <input type="range" min="30" max="90" value={textPositions.course.y} onChange={e => setTextPositions({...textPositions, course: {...textPositions.course, y: Number(e.target.value)}})} className="w-full mt-1" />
                    </div>
                    <div>
                      <label>Date Y Pos ({textPositions.date.y}%)</label>
                      <input type="range" min="40" max="95" value={textPositions.date.y} onChange={e => setTextPositions({...textPositions, date: {...textPositions.date, y: Number(e.target.value)}})} className="w-full mt-1" />
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-2 w-full">
                    ...
                    <div className="text-2xl lg:text-3xl font-serif font-bold text-[#08060d] border-b-2 border-[#aa3bff] pb-1 px-4 my-2">
                      {recipients[0] ? recipients[0][columnMapping.name] || 'Daniel Kombou' : 'Daniel Kombou'}
                    </div>
                    <div className="text-base lg:text-lg text-gray-700 font-medium">
                      {recipients[0] ? recipients[0][columnMapping.course] || 'Software Engineering' : 'Software Engineering'}
                    </div>
                    <div className="text-xs text-gray-500 mt-2">
                      {recipients[0] ? recipients[0][columnMapping.date] || 'August 2, 2026' : 'August 2, 2026'}
                    </div>
                  </div>
                </div>
